...
void Dyna::Simulate(){	
	...
	float *tp, *firstfail;
	if(debug){
		tp = new float[800*440];
		ReadTrace* tr = new ReadTrace();
		tr->readtomem("tp.dat",tp);
		tr->closefile();
		delete tr;

		firstfail = new float[800*440];
		for(int i=0; i<800; i++){
		...
		for(int globaliter=0; globaliter<randomsize; globaliter++)
		{
			...
			if(dag->type == montage){
				for(int i=0; i<4; i++) {
					...
					dag->g[i].readyCountdown = -1;
					dag->g[i].restTime = 0;				
				}
				for(int i=4; i<20; i++){
					dag->g[i].status = not_ready;
					dag->g[i].readyCountdown = -1;
					dag->g[i].restTime = 0;	
				}
			}else{
					...
					if(curr_task->readyCountdown == -1)//
					{
						...
						if(!suitfind){
							...
							for(int ti = 0; ti<types&& !suitfind; ti++){
								...
								for(int j=0; j<size&& !suitfind; j++)
								{
									float runtime = VMTP[ti][j]->life_time;
			...
			vp = vertices(dag->g);
			float executiontime = dag->g[*(vp.second-1)].end_time - dag->arrival_time;
			if(executiontime == 0)
				break;
