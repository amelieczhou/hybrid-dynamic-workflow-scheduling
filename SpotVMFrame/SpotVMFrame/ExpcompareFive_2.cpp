...
float OnDemandLag = 90;//seconds//10;//0.5;
float SpotLag = 60;//seconds//1;
bool NoSpotVM = true;
...
{	
	...
	float meet_dl = atof(argv[13]); //the deadline meet rate submitted by the user
	lambda = atof(argv[14]);
	num_jobs = atoi(argv[15]);
	randomsize = atoi(argv[16]);
